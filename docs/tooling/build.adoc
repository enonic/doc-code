...
NOTE: Upgrading from XP 7? See <<../upgrade/xp7#, Upgrading Enonic apps from XP 7 to XP 8>>.

== Main builder

Enonic apps and libraries use https://gradle.org/[Gradle] as the main build tool.
...
<3> Dependencies on libraries. `api.script` is the entry point for the server-side JavaScript/TypeScript APIs.
...
In addition to running XP in dev mode, your application may contain files that require compilation (including transpilation). This means the files must be built for every change. For instance, if you use TypeScript, these files won't be picked up by default.
