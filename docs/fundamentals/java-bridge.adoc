...
WARNING: image:xp-810.svg[XP 8.1.0,opts=inline] Register disposers while the application is starting — from <<main#, `main.ts`>>, or from a module it loads during start-up. A registration made later, such as from a component implementation the first time it is loaded, is ignored on the GraalJS engine and logged: only a disposer belonging to the start-up context survives to the stop phase. Nashorn is more forgiving today, but registering from anywhere else is not portable between engines.

NOTE: GraalJS is a image:preview.svg[Preview,opts=inline] preview engine and *not for production* — see <<runtime#graaljs, GraalJS preview>>. Follow the start-up rule above regardless: it is the portable one.
