...
Server-side code in an Enonic app reaches a handful of names it never declares and never requires: `app`, `log`, `require`, `resolve`, `+__+`, `exports` and `module`. Only one of them is a true global.
...
* *They are per module.* Each module body gets its own `exports` and `module`, and a `require` and `resolve` bound to that module's own location — which is what makes a relative path resolve against the file that wrote it.
...
``module`` describes the module currently being loaded. `module.id` is its resource key, and `module.exports` is the same object as `exports` — reassign `module.exports` when a module's whole export is a single function or value rather than a set of named ones.

[#esm]
== ECMAScript modules are not supported

`import` and `export` statements do not work at runtime. XP loads every module through the CommonJS wrapper shown <<module-scope, above>> and evaluates it as a classic script, so an `import` statement is a syntax error rather than a module declaration.

This holds on both engines, the image:preview.svg[Preview,opts=inline] <<runtime#graaljs, GraalJS preview>> included: GraalJS implements ES modules perfectly well, but XP never asks it to evaluate one.
