...
* *They are not on `globalThis`.* `globalThis.log` is `undefined`, and so are `require`, `resolve`, `+__+`, `exports` and `module`. Only `globalThis.app` resolves. Inside a module `typeof require` still answers `"function"` — it is the parameter that answers, not a global — so a bundled npm package probing its environment through `globalThis` will not find what it is looking for.
