...
Start:: XP starts the app on every cluster node. If the bundle contains <<main#, `main.ts`>> at `src/main/resources/main.ts`, it is loaded and its top-level code runs once per node. image:xp-810.svg[XP 8.1.0,opts=inline] Nothing else in the app runs until it returns, so start-up cannot race the code it initializes. `+__.disposer()+` registrations made during start are remembered for the stop phase; registering one later is not dependable.
...
For deployment, scaling, and isolation details that are platform-level rather than developer-level, see the https://developer.enonic.com/docs/platform[XP platform documentation].
...
The file format, location, and reload behavior are documented in the https://developer.enonic.com/docs/platform/xp8/config[XP platform documentation].
