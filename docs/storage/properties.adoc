...
Properties contain the actual data inside a <<../libraries/lib-node#, node>>.
...
Applications can override the default mappings per property via the `_indexConfig` system property. For the full configuration reference — config options, templates, property-path patterns, and the language list for stemming — see <<indexing#, Indexing>>.
...
Node permissions are also flattened into indexed `_permissions_*` system properties at write time. See <<permissions#_querying_by_permission, Permissions — Querying by permission>> for the full list and usage.
