...
Properties contain the actual data inside a https://developer.enonic.com/docs/platform/stable/storage/nodes[node].
...
In the example above, the property `person` is of the <<#value_types, value type>> `Set`.
...
Applications can override the default mappings per property via the `_indexConfig` system property. For the full configuration reference — config options, templates, property-path patterns, and the language list for stemming — see <<storage/indexing#, Indexing>>.
...
Node permissions are also flattened into indexed `_permissions_*` system properties at write time. See <<storage/permissions#_querying_by_permission, Permissions — Querying by permission>> for the full list and usage.
