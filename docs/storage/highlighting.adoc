...
By default, the highlighting will wrap highlighted text inside `<em></em>` tag. This can be controlled by setting `preTag` and `postTag`, for example:
...
If `numberOfFragments` is set to 0 then no fragments are produced, instead the entire content of the property is returned, and of course it is highlighted.
...
----
{
    ...
    "highlight" : {
        ...
        "properties" : {
            ...
            "description" : { "preTag" : "<tag1>", "postTag" : "</tag1>" },
