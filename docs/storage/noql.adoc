...
WARNING: NoQL is deprecated. Use the <<storage/querying#, Query DSL>> for new code. This reference is kept for maintaining existing applications.
...
Query DSL is an AST (Abstract Syntax Tree) expressed in JSON to define query expressions. See <<storage/querying#, documentation>> for further explanation and examples.
...
For both the integer comparison and fulltext expression to work, the weight, and article properties need to be <<storage/indexing#, indexed>> properly.
...
NOTE: `instant`, `dateTime`, `localDateTime`, `date` functions will force search in `datetime` index, meanwhile for `time` function  usual `text` index will be used. Using expressions without functions perform a search in `text` index for any time-related field. See more about indexing <<storage/indexing#, there>>.
...
NOTE: Stemming is language-dependent, so <<storage/indexing#languages,language>> must be set either on the content or directly in the node indices via indexConfig.
...
language:: Content language that was used for stemming. List of <<storage/indexing#languages, supported languages>>
...
To use DSL sort check <<storage/querying#sort, documentation>>.
...
Sort DSL is an AST (Abstract Syntax Tree) expressed in JSON to define sort expressions. See <<storage/querying#sort, documentation>> for further explanation and examples.
