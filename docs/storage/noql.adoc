...
WARNING: NoQL is deprecated. Use the <<querying#, Query DSL>> for new code. This reference is kept for maintaining existing applications.

== Introduction

The Node Query Language (NoQL) is a SQL-inspired string format for querying nodes. It runs on the same query engine as the Query DSL, but is no longer the recommended entry point — the DSL is programmatically composable and covers the full feature set.

A NoQL statement is composed from two parts: <<query, query>> and <<sort, sorting>>. The result is a set of identifiers for matching nodes; fetch the node data through a separate request. Selectors, joins, and update statements are not supported.
...
Query DSL is an AST (Abstract Syntax Tree) expressed in JSON to define query expressions. See <<querying#, documentation>> for further explanation and examples.
...
NOTE: `instant`, `dateTime`, `localDateTime`, `date` functions will force search in `datetime` index, meanwhile for `time` function  usual `text` index will be used. Using expressions without functions perform a search in `text` index for any time-related field. See more about indexing <<indexing#, there>>.
...
You can boost the relevance score on a per-field basis by appending a weight-factor ``^N`` to a field name (for example ``displayName^5``).
The weight is a *positive multiplier* on the field's score contribution, compared to the default weight of ``1``: a value above ``1`` makes the field contribute more, a value between ``0`` and ``1`` makes it contribute less.
The contribution always stays positive — a match adds to the score, it is never a penalty. Negative weights are not allowed.
Decimal weights (for example ``displayName^1.5``) are supported from XP 8.0.0; in earlier versions the field weight must be an integer.
...
To use DSL sort check <<querying#sort, documentation>>.
...
Sort DSL is an AST (Abstract Syntax Tree) expressed in JSON to define sort expressions. See <<querying#sort, documentation>> for further explanation and examples.
