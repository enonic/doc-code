...
NOTE: Search could be done by an expression with particular type only for analyzed fields with compatible <<properties#value_types, value type>> and not prohibiting type mapping.
...
Time expression converts expression as <<properties#value_types, LocalTime>> string and makes a search for a field indexed as 'string'.

===== dateTime

DateTime expression makes a search for fields indexed as 'datetime' (`Instant`, `LocalDate`, `LocalDateTime` <<properties#value_types, value types>>). If time is not specified it will be set to zero. If timezone is not specified it will be set to UTC.
...
NOTE: You can boost the relevance score on a per-field basis by appending a weight-factor `^N` to a field name (for example `_name^3`). The weight is a *positive multiplier* on the field's score contribution, compared to the default weight of `1`: a value above `1` makes the field contribute more, a value between `0` and `1` makes it contribute less. The contribution always stays positive — a match adds to the score, it is never a penalty. Negative weights are not allowed. Decimal weights (for example `_name^1.5`) are supported from XP 8.0.0; in earlier versions the field weight must be an integer.
...
===== filter
Equivalent to <<must>>, but faster — `filter` skips relevance scoring, so matches don't contribute to `_score`.

Any expression or compound documented above can be used inside `filter`; there is no filter-specific syntax.
...
TIP: Migrating from the legacy <<filters#, filters>> API? See the <<filters#legacy_filter_migration, mapping table>>.
...
A `boost` is a *positive multiplier* applied to the score contribution of a clause, compared to the default of `1`:
...
IMPORTANT: A `boost` between `0` and `1` is *not* a negative boost. "Less" here means less than at `boost` `1` — the contribution stays positive, so a match still adds to the score and always ranks a matching document at or above one that does not match. It never penalizes documents that match.

Boosting can be applied in three different places. They all use a positive multiplier, but are configured differently:

[cols="1,2,1", options="header"]
|===
|Boost type |How to apply |Decimals

|<<field_boost, Field boost>>
|Append `^N` to a field name in a `fields` array.
|From XP 8.0.0 (integer only before)

|<<expression_boost, Expression boost>>
|Set the `"boost"` property on an expression (`term`, `fulltext`, `range`, …).
|Yes

|<<group_boost, Compound (group) boost>>
|Set the `"boost"` property on a `boolean` query.
|Yes
|===
...
----
The `title` field contributes three times as much as `body` to the score.
...
----
<1> Boost above `1`, increasing this clause's contribution to the score.
<2> Boost between `0` and `1`, reducing this clause's contribution. The match still adds to the score — this is not a penalty.

[#group_boost]
To boost a bunch of expressions together, wrap them in an inner boolean query:

.Compound (group) boost
[source,json]
----
{
  "boolean": {
...
NOTE: Combining group boosting (a `boost` on the wrapping `boolean`) with single-term boosting (a `boost` on the inner expressions) is allowed — neither is ignored and no error or warning is raised. However, both multipliers are applied and combine into the effective score, which is easy to misjudge. For predictable relevance, prefer boosting at a single level.
...
In addition to the default UNICODE sorting, text fields can be sorted using locale-aware collations, such as DUCET or language-specific collations. This allows for more natural sorting of text for specific languages.

NOTE: Locale-aware sorting is only supported for text fields that are indexed with a language-specific analyzer. The language for sorting can be specified using the `language` property in the sort definition.
