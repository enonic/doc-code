...
NOTE: Search could be done by an expression with particular type only for analyzed fields with compatible <<storage/properties#value_types, value type>> and not prohibiting type mapping.
...
Time expression converts expression as <<storage/properties#value_types, LocalTime>> string and makes a search for a field indexed as 'string'.

===== dateTime

DateTime expression makes a search for fields indexed as 'datetime' (`Instant`, `LocalDate`, `LocalDateTime` <<storage/properties#value_types, value types>>). If time is not specified it will be set to zero. If timezone is not specified it will be set to UTC.
...
|string[]
|true
|List of <<storage/noql#propertyPath, propertyPaths>> to match.
...
List of <<storage/indexing#languages, supported languages>>
...
TIP: Migrating from the legacy <<storage/filters#, filters>> API? See the <<storage/filters#legacy_filter_migration, mapping table>>.
