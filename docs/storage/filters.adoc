...
Filters work differently from <<storage/querying#, queries>>.
...
WARNING: Filters are deprecated and will be removed in future XP versions. Use <<storage/querying#_filter, query filtering>> instead — see the <<legacy_filter_migration, mapping table below>> for how each filter type translates to the DSL.

[#legacy_filter_migration]
== Migrating to the Query DSL

Each legacy filter has a direct equivalent inside `boolean.filter` (or `boolean.mustNot`) in the <<storage/querying#, Query DSL>>:
...
|`exists`
|<<storage/querying#exists, exists>> expression inside `filter`

|`notExists`
|<<storage/querying#exists, exists>> expression inside `mustNot`

|`hasValue`
|<<storage/querying#in, in>> expression inside `filter`

|`ids`
|<<storage/querying#in, in>> expression on the `+_id+` field inside `filter`
