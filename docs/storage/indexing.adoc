...
The "orderby" index mapping lets us sort text and numbers in a natural way across properties with different value types.
...
Use the <<storage/querying#stemmed, stemmed>> query expression (or the legacy <<storage/noql#stemmed, stemmed()>> NoQL function) to query data based on these indices.
