...
Properties are mapped to specific indexes based on their <<properties#value_types, value type>> and the node's <<Index config>> — as illustrated below:
...
Used by the <<querying#fulltext, fulltext>> query expression (and the legacy <<noql#fulltext, fulltext()>> NoQL function).
...
Used by the <<querying#stemmed, stemmed>> query expression (and the legacy <<noql#stemmed, stemmed()>> NoQL function).
...
Read more about collation in the <<querying#sort, Sort DSL>> section.
...
By default, properties are indexed based on their specific <<properties#value_types, value type>>.
...
decideByType:: If true, indexing is done based on the property's <<properties#value_types, value type>>. I.e. numeric values are indexed as both `text` and `number`.
fulltext:: Values are indexed as `analyzed`, enabling the <<querying#fulltext, fulltext>> query expression
...
For examples on querying stemmed fields, visit the <<querying#stemmed, Query DSL>> (or the legacy <<noql#stemmed, NoQL>> documentation).
...
For examples on sorting text fields using collation, visit the <<querying#sort, Query DSL>> (or the legacy <<noql#sort, NoQL>> documentation).
