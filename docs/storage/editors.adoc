...
|===
| Function | Receives | Notes
| <<../libraries/lib-content#update, `lib-content.update`>>    | Full content, but only `displayName`, `language`, `data`, `x` and `page` are written back | Validates against the content type unless `requireValid: false`. Other properties assigned in the editor are silently ignored.
...
Pair it with a query (see <<querying#, Query DSL>>) to walk the affected contents and apply the change to each:
...
* *No-op edits are skipped.* If the editor returns its input unchanged, the engine compares the result with the stored version and writes nothing — no new version, no timestamp bump. This holds for both `lib-node` and `lib-content`.
* *A genuine `lib-content.update` resets workflow state.* Any update that actually changes the content moves it back to `IN_PROGRESS` and refreshes `valid`, `modifier` and `modifiedTime`; `patch` leaves all of these untouched. Publishing requires `READY` — mark the content ready with <<../libraries/lib-content#updateworkflow, `updateWorkflow`>> after editing.
...
* *Editors run as the calling principal.* For changes that exceed the caller's permissions — bulk fixes, migrations — wrap the editor block in <<../libraries/lib-context#run, `lib-context.run`>> with a service principal. See <<permissions#running_as_a_different_principal, Permissions>> for the pattern.
