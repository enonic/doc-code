...
* *Editors run as the calling principal.* For changes that exceed the caller's permissions — bulk fixes, migrations — wrap the editor block in <<../libraries/lib-context#run, `lib-context.run`>> with a service principal. See <<permissions#_running_as_a_different_principal, Permissions>> for the pattern.
