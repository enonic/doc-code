...
Already on XP 8? See <<../upgrade#, Upgrading Enonic apps>> for the changes between XP 8 releases.

NOTE: To run this upgrade automatically with an AI coding agent, use the https://github.com/enonic/agent-toolkit[Enonic Agents Toolkit] — it includes a skill that drives agents like Claude Code, Codex, Gemini and Co-pilot (or another compatible agent) through the procedure below. The skill reads this page as its source, so the steps documented here are authoritative whether you upgrade by hand or via an agent.

[#terminology]
== Terminology and concept changes

XP 8 renames or reframes several XP 7 concepts — "engine" is retired in favour of *service*, controllers become plain *functions*, services become *Universal APIs*, and more. When migrating, or when reconciling older XP 7-era examples or AI-generated code with current conventions, consult the full map in the release notes: https://developer.enonic.com/docs/platform/stable/release#naming[A paradigm shift in naming].

The actionable directory, descriptor, and method renames are detailed in the steps below.

== Preparations

Before you start the upgrade procedure:

. Make sure you have https://developer.enonic.com/docs/enonic-cli/stable[Enonic CLI] installed
...
`displayName`, `url`, `vendorName` and `vendorUrl` must be moved to the new _src/main/resources/enonic.yaml_

NOTE: `displayName` is now called `title` in _enonic.yaml_).
...
Note that input configuration is no longer nested: keys inside XP 7's `<config>` element become flat properties of the input in YAML. There is no `config:` wrapper — a nested `config:` block is not recognized and its settings are ignored.

XP 7's `<display-name-label>` element is renamed to `displayNamePlaceholder`. Unknown keys in the YAML descriptor — including the literal `displayNameLabel` — fail the parse, so a mistranslated descriptor is rejected at registration rather than silently ignored.
...
<content-type xmlns="urn:enonic:xp:model:1.0">
  ...
  <form>
    ...
    <input name="author" type="ContentSelector">
      ...
      <occurrences minimum="0" maximum="1"/>
      <config>
        <allowContentType>com.example.app:author</allowContentType>
      </config>
    </input>
  </form>
</content-type>
...
`resolve` function no longer uses `src/main/resources/site` as its first root folder. Only `src/main/resources` is used as the root. If your modules relied on resolving paths relative to the `site` folder, update them to use full paths from the resources root.
...
`publish` no longer accepts the `sourceBranch` and `targetBranch` parameters — publishing always pushes from `draft` to `master`, and keys are resolved against `draft` regardless of the current context branch. XP 7 code passing these parameters gets no error: they are silently ignored, so a former `master` → `draft` push now runs as a normal `draft` → `master` publish. For branch-to-branch operations in custom repositories, use `push` in `lib-node`.
...
`submit` method is removed. Its drop-in replacement `executeFunction` is deprecated in turn — convert the callback to a <<../background/tasks#, named task>> and use `submitTask` instead. `executeFunction` is additionally unsupported on GraalJS, which is a image:preview.svg[Preview,opts=inline] preview engine and *not for production*.
...
Migrating to Universal API means the existing service endpoint URLs will change. Services were previously mounted under `/_/service/<app-name>/<service-name>`, while Universal APIs are contextually available directly under `/_/<app-name>:<api-name>` in addition to the new `/api` mount point. 
...
* https://developer.enonic.com/docs/lib-asset/stable[lib-asset] — best fit for site applications, where it integrates with the site service and content context.
