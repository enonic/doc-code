...
TypeScript types come from the `@enonic-types/lib-content` package, installed at the version of XP your project builds against — see <<../tooling/typescript#, TypeScript>>.
...
| data | <<../libraries/types#byte-source, ByteSource>> | Stream with the binary data for the attachment.
|===
...
| page | object | *Optional.* The content's <<../libraries/types#page-component, page>> — see <<pages-and-components, Pages and components>>.
...
| workflow | <<../libraries/types#workflow, Workflow>> | *Optional.* Workflow information to use. Defaults to `{ state: "READY" }`.
|===

[.lead]
Returns

*object* : (<<../libraries/types#content, Content>>) The content that was created
...
*object* : (<<../libraries/types#content, Content>>) The media content that was created
...
*object* : (<<../libraries/types#content, Content>>) The media content that was updated, or `null` if it was not found
...
*object* : (<<../libraries/types#content, Content>>) The content fetched from the repository, or `null` if it was not found
...
*object* : A map of <<../libraries/types#attachment, Attachment>> objects keyed by attachment name, or `null` if the content was not found
...
*object* : (<<../libraries/types#byte-source, ByteSource>>) Stream of the attachment data, or `null` if the content or attachment was not found
...
WARNING: Deprecated since XP 8.1 — use <<query, `query`>> with `parent` instead. It accepts the same path or id, applies the same child order of the parent where `sort` is omitted, and additionally supports filters, content types, aggregations and highlighting.
...
*object* : (<<../libraries/types#content, Content>>) The parent site, or `null` if the content has none
...
| page | object | The <<../libraries/types#page-component, page>> as stored — see <<pages-and-components, Pages and components>>. Applied key by key: `descriptor`, `template`, `config` and `regions` each replace what is stored when present, keep it when left out, and clear it when set to `null`. Assign `null` to the whole `page` to remove it.
|===
...
*object* : (<<../libraries/types#content, Content>>) The updated content, or `null` if it was not found
...
Patches a content. This is an operations-level write, meant for content upgrades, migrations and repairs — as opposed to the everyday editing that <<update, update()>> is for.

[WARNING]
====
`patch()` bypasses the checks that keep content consistent. It stores exactly what the patcher returns: no validation against the content type, no content processors, no change detection, and no stamping of `modifier`, `modifiedTime` or the workflow state — everything `update()` does on a genuine change. It also writes to every branch listed in `branches` in a single call, so a mistake can land in `master` without ever passing through publishing.

Use it only from upgrade tasks and maintenance scripts, and use `update()` for anything an editor could have done in Content Studio. Access is restricted to users with the `admin` or `cms.admin` role.
====

What the bypass buys: `patch()` can set the fields those mechanisms normally own — `creator`, `createdTime`, `modifier`, `modifiedTime`, `valid`, `workflow`, `publish`, `originProject`, … — and a fix applied to `draft` and `master` together needs no republish.
...
| page | object | The <<../libraries/types#page-component, page>> — see <<pages-and-components, Pages and components>>. Replaces the stored page as a whole.
...
| workflow | object | A JSON object containing <<../libraries/types#workflow, workflow>> state information.
...
| validationErrors | <<../libraries/types#validation-error, ValidationError>>[] | An array of objects describing validation errors.
...
| data | <<../libraries/types#byte-source, ByteSource>> \| string | *Optional.* Data to use. A string is stored as its UTF-8 bytes.
|===
...
*object* : (<<../libraries/types#content, Content>>) The content that was moved or renamed
...
| parent | string | *Optional.* image:xp-810.svg[XP 8.1.0,opts=inline] Path or id of a content to whose direct children the query is restricted. Where `sort` is not specified, the children are returned in the child order of the parent. A parent that does not exist matches nothing. A path is relative to the content root of the calling context; as the same API also serves the archive, an identical path denotes a different content depending on the context.
| recursive | boolean | *Optional.* image:xp-810.svg[XP 8.1.0,opts=inline] Matches every descendant of `parent` rather than its direct children only. Requires `parent`. The child order of the parent is not applied to a subtree, since it orders siblings rather than levels, so specify `sort` where the order of a recursive result is significant. Defaults to `false`.
| returns | string \| string[] | *Optional.* image:xp-810.svg[XP 8.1.0,opts=inline] What each hit carries: `'contents'` for the whole content, the default; `'ids'` for id and score alone; or a list of content field names — `+_name+`, `+_path+`, `displayName`, `type`, `creator`, `modifier`, `createdTime`, `modifiedTime`, `owner`, `language` — supplied as `fields` on the hit. Any other name is an error, as is an empty list; request `'ids'` where no fields are required. Every shape other than the default is answered without reading the contents at all, so it is the cheaper way to ask when the whole content is not needed.
| start | number | *Optional.* Start index (used for paging). Defaults to `0`.
| count | number | *Optional.* Number of contents to fetch. Defaults to `10`.
...
.Every descendant that is an article, with a few fields of each
[source,typescript]
----
import {query} from '/lib/xp/content';
...
The `page` property of <<create, create()>>, <<update, update()>> and <<patch, patch()>> takes the same tree that <<get, get()>> returns for a content's page — a <<../libraries/types#page-component, PageComponent>> whose `regions` hold <<../libraries/types#layout-component, layouts>>, <<../libraries/types#part-component, parts>> and <<../libraries/types#text-component, text>>. It takes one of three forms:

* `{}` — no page of its own. The content renders through whatever page template its content type resolves to.
* `{ type: 'page', template: '<id>' }` — a specific page template, referenced by the id of the template content.
* A full page — a `descriptor`, its `config`, and `regions` holding the components.

`descriptor` and `template` are mutually exclusive; a page with both is rejected. Each component needs a `type`; a component without one, or with a type other than `layout`, `part`, `text` or `fragment`, is rejected. Regions are keyed by name, and a layout carries its own `regions` in the same shape as the page. `path` — and a `name` inside a region — are set by XP when the page is stored and are ignored on input.

The tree is a view. As raw node properties, a page is persisted as a flat `components` list — one entry per component, carrying its `path`, its `type`, and a block named after the type — and lib-content converts between the two on every read and write. The flat list is what Guillotine's `components` field exposes, and what you see when reading the content's node directly; its shape is described in the https://developer.enonic.com/docs/cms/stable/pages/indexing[CMS documentation]. It is a read-only view for application code: always write content through this library, which validates the page against its descriptors and applies the rest of the content logic — never through <<../libraries/lib-node#, lib-node>>, which bypasses all of it. Templates are the other difference between the reading APIs: <<get, get()>> returns the page as stored, so a content that uses a template has `template` and no regions, while <<../libraries/lib-portal#getcontent, `getContent()`>> in lib-portal returns the page as rendered, with the template's components in place.

NOTE: A property of the wrong type is dropped silently rather than rejected — a `descriptor` that is not a string leaves the page without a descriptor, and a `config` that is not an object leaves the component without one. Check the returned content when a page does not come out as expected.

.Creating a content with a page
[source,typescript]
----
import {create} from '/lib/xp/content';

const article = create({
    parentPath: '/mysite/articles',
    displayName: 'Hello',
    contentType: 'com.myapp:article',
    data: {
        title: 'Hello'
    },
    page: {
        type: 'page',
        descriptor: 'com.myapp:article',
        config: {
            theme: 'dark'
        },
        regions: {
            main: {
                components: [
                    {
                        type: 'layout',
                        descriptor: 'com.myapp:two-column',
                        regions: {
                            left: {
                                components: [
                                    {
                                        type: 'part',
                                        descriptor: 'com.myapp:teaser',
                                        config: {
                                            limit: 3
                                        }
                                    }
...
The stored page comes back with a `path` on every component — `/main/0` for the layout, `/main/0/left/0` for the part — which is how <<../libraries/lib-portal#, `getComponent()`>> and component URLs address them.

The three writing functions treat the object differently. `create()` and `patch()` take it whole: what you pass is the page. `update()` applies it *key by key* — the editor receives the page as stored, and of `descriptor`, `template`, `config` and `regions`, each key present in what the editor returns replaces the stored value, a key left out keeps it, and a key set to `null` clears it. Since `descriptor` and `template` are exclusive, switching from one to the other means clearing the other in the same edit, or the update is rejected. Assigning `null` to `page` itself removes the page — it reads back as `{}` — so the content falls back to the template its content type resolves to.

There is no way to address a single component; the unit written is the tree. In practice the editor's round trip makes this a small edit — walk to the component in the stored tree, change it, and return the tree. The stored shape is accepted as input as it is, since `path` and a region's `name` are ignored.

.Editing one component and appending another
[source,typescript]
----
import {update} from '/lib/xp/content';

update({
    key: '/mysite/articles/hello',
    editor: (c) => {
        const layout = c.page.regions.main.components[0];
        layout.regions.left.components[0].config.limit = 5;
        c.page.regions.main.components.push({
            type: 'text',
...
.Switching a page from its own descriptor to a template
[source,typescript]
----
import {update} from '/lib/xp/content';
...
update({
    key: '/mysite/articles/hello',
    editor: (c) => {
        c.page.template = '79e21db0-5b43-45ce-b58c-6e1c420b22bd';
        c.page.descriptor = null;
        c.page.regions = null;
        c.page.config = null;
        return c;
    }
...
Clearing `regions` and `config` as well hands rendering entirely to the template. A page that keeps `template` and also carries `regions` or `config` of its own is valid — at render time they take the place of the template's, which is how a templated page is customized.

A page edit is a content change like any other: through `update()` it resets the workflow state to `IN_PROGRESS`, so the content will not <<publish, publish>> until it is marked ready again with <<updateworkflow, updateWorkflow()>>; through `patch()` the workflow state is left as it was.
...
| form | <<../libraries/types#form-item, FormItem>>[] | Form schema — an array of form items.
...
|===
| Name | Type | Description
| data | <<../libraries/types#byte-source, ByteSource>> | Stream with the binary data for the icon. Read it with `lib-io`.
...
A publish schedule. Same shape as <<../libraries/types#publish-info, PublishInfo>> without the `first` field.
...
|===
| Name | Type | Description
| principal | <<../libraries/types#principal-key, PrincipalKey>> | Principal the entry applies to (for example, `role:system.admin`, `user:system:su`).
...
| hits | <<../libraries/types#content, Content>>[] \| <<content-hit, ContentHit>>[] | The contents on this page — or, image:xp-810.svg[XP 8.1.0,opts=inline] where `query` asked for another shape through `returns`, one <<content-hit, ContentHit>> per hit instead of the content itself.
...
image:xp-810.svg[XP 8.1.0,opts=inline] A hit of a <<query, `query`>> that asked for `returns: 'ids'` or a list of field names. The content itself is not read to answer such a query, so a hit carries nothing beyond what was asked for.
...
| score | number | Score of the hit, as for any other query.
| fields | object | *Optional.* The requested field values, keyed by the names asked for. Present only where `returns` named fields rather than `'ids'`. A field the content does not have is absent rather than null.
|===
...
| user | <<../libraries/types#principal-key, PrincipalKey>> | Principal that performed the operation.
...
| content | <<../libraries/types#content, Content>> | The patched content in that branch, or `null` if the content does not exist there.
|===
...
|===
| Name | Type | Description
| content | <<../libraries/types#content, Content>> | The content with its updated metadata.
|===
...
|===
| Name | Type | Description
| content | <<../libraries/types#content, Content>> | The content with its updated workflow state.
|===
