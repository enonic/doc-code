...
TypeScript types come from the `@enonic-types/lib-content` package, installed at the version of XP your project builds against — see <<../fundamentals/typescript#, TypeScript>>.
...
| hits | <<../fundamentals/types#content, Content>>[] \| <<content-hit, ContentHit>>[] | The contents on this page — or, image:xp-810.svg[XP 8.1.0,opts=inline] where `query` asked for another shape through `returns`, one <<content-hit, ContentHit>> per hit instead of the content itself.
