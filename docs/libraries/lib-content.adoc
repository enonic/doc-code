...
| hits | <<../fundamentals/types#content, Content>>[] | The contents on this page. image:xp-810.svg[XP 8.1.0,opts=inline] Where `query` was asked for another shape through `returns`, a hit is `{id, score}` instead — with `fields` alongside them when field names were requested, keyed by the names asked for. A field a content does not have is absent from `fields` rather than null.
