...
TypeScript types come from the `@enonic-types/lib-value` package, installed at the version of XP your project builds against — see <<../tooling/typescript#, TypeScript>>.
...
`binary()` takes two positional arguments: `name` (string — the binary name) and `stream` (<<../libraries/types#byte-source, ByteSource>> — the binary stream).
...
| `getByteSource()` | <<../libraries/types#byte-source, ByteSource>> | The underlying binary stream.
|===
