...
TypeScript types come from the `@enonic-types/lib-io` package, installed at the version of XP your project builds against — see <<../tooling/typescript#, TypeScript>>.
...
Takes a single `key` argument — a string path or a <<../libraries/types#resource-key, ResourceKey>> identifying the resource.

[.lead]
Returns

*object* : (<<../libraries/types#resource, `Resource`>>) A resource handle exposing `exists()`, `getSize()`, and `getStream()`.
...
Takes a single `stream` argument — a <<../libraries/types#byte-source, ByteSource>> to measure.
...
*object* : (<<../libraries/types#byte-source, `ByteSource`>>) A new stream containing the UTF-8 bytes of the input string.
...
Takes two positional arguments: a `stream` (<<../libraries/types#byte-source, ByteSource>>) to read from, and a `func` callback `(line: string) => void` called once per line.
...
Takes a single `stream` argument — a <<../libraries/types#byte-source, ByteSource>> to read from.
