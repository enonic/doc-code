...
A `Content-Security-Policy` header returned directly from a controller response is not sent verbatim: it is folded into the request policy (parsed as if passed to `resetTo()`), so the policy stays the single source of truth and later contributions still apply on top. A `Content-Security-Policy-Report-Only` header is folded the same way into the request's report-only policy — and since the builder governs the enforced policy, returning that header is how a report-only policy is set.
