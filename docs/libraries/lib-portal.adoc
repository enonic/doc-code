...
Sources are validated: a value containing `;` or whitespace (a policy-injection attempt) throws, and a hand-supplied `'nonce-…'` source is rejected — only `nonceScriptSrc()` / `nonceStyleSrc()` mint the request nonce.
...
|===
| Method | Returns | Description
| `nonceScriptSrc()` | `string` | Wires the request nonce into `script-src` and returns its value, to stamp on inline `+<script nonce="...">+` tags. Stable for the whole request.
| `nonceStyleSrc()` | `string` | Wires the request nonce into `style-src` and returns its value, to stamp on inline `+<style nonce="...">+` tags. The same value as `nonceScriptSrc()`.
