...
TypeScript types come from the `@enonic-types/lib-portal` package, installed at the version of XP your project builds against — see <<../tooling/typescript#, TypeScript>>.
...
Generates a URL pointing to a <<../web/apis#, Universal API>>. Picks the right URL form based on the calling context — a contextual URL under the active service when called from a site, webapp, or admin tool, and `/api/` otherwise.
...
image:xp-810.svg[XP 8.1.0,opts=inline] Returns the request-scoped *enforced* <<csp-type, Content Security Policy>> builder for the current portal request. A portal request exists in site pages and components, webapps, admin tools and APIs alike, so the policy can be declared from any of them, and a Java <<../web/extensions/processors#, extension response processor>> contributes to the same policy. Its directives are composed into the `Content-Security-Policy` header emitted on the response. For the report-only companion, use <<cspReportOnly, `cspReportOnly()`>>. Takes no arguments.
...
+*object* : (<<../libraries/types#component, `Component`>>) The current component, or `null` if there is no component in the current context.+
...
The content's `page` is the page as rendered: where the content uses a page template, the template's components are in place. For the page as stored, and for how the tree relates to the flat `components` list XP persists, see <<../libraries/lib-content#pages-and-components, Pages and components>>.

The content follows the request's `contentPath`, so when a <<../web/sites/mappings#reroute, site mapping filter>> has re-routed rendering, this returns the content the request was re-routed to.

[.lead]
Returns

+*object* : (<<../libraries/types#content, `Content`>>) The current content, or `null` if there is no content in the current context.+
...
+*object* : (<<../libraries/types#byte-source, `ByteSource`>>) Stream of multipart item data, or `null` if the item does not exist.+
...
A site is a specialized <<../libraries/types#content, `Content`>> of type `portal:site`. In addition to the standard content properties, its `data` carries the site's app configurations.
