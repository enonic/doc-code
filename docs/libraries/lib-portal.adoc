...
Returns the request-scoped <<csp-type, Content Security Policy>> builder for the current portal request. Its directives are composed into the `Content-Security-Policy` header emitted on the response. Takes no arguments.
...
NOTE: Setting the `Content-Security-Policy` header directly is a legacy path, kept only for compatibility with existing apps. New code should declare the policy through `csp()` instead.
...
A request-scoped Content Security Policy builder, obtained from <<csp, `csp()`>>. The same instance backs the whole request, so every controller, layout, part and processor contributes to the one `Content-Security-Policy` header emitted on the response. Every method except the nonce getters returns the same `Csp` instance, so calls chain.
...
| `defaultSrc(...sources)` | `Csp` | Unions sources into `default-src`, the fallback for directives left unset.
| `scriptSrc(...sources)` | `Csp` | Unions sources into `script-src`.
| `styleSrc(...sources)` | `Csp` | Unions sources into `style-src`.
| `imgSrc(...sources)` | `Csp` | Unions sources into `img-src`.
| `fontSrc(...sources)` | `Csp` | Unions sources into `font-src`.
| `connectSrc(...sources)` | `Csp` | Unions sources into `connect-src` (fetch, XHR, WebSocket, etc.).
| `mediaSrc(...sources)` | `Csp` | Unions sources into `media-src`.
| `objectSrc(...sources)` | `Csp` | Unions sources into `object-src`.
| `frameSrc(...sources)` | `Csp` | Unions sources into `frame-src`.
| `workerSrc(...sources)` | `Csp` | Unions sources into `worker-src`.
| `manifestSrc(...sources)` | `Csp` | Unions sources into `manifest-src`.
| `childSrc(...sources)` | `Csp` | Unions sources into `child-src`.
| `scriptSrcElem(...sources)` | `Csp` | Unions sources into `script-src-elem` (governs `+<script>+` elements).
| `scriptSrcAttr(...sources)` | `Csp` | Unions sources into `script-src-attr` (governs inline event handlers).
| `styleSrcElem(...sources)` | `Csp` | Unions sources into `style-src-elem` (governs `+<style>+` and `+<link rel=stylesheet>+`).
| `styleSrcAttr(...sources)` | `Csp` | Unions sources into `style-src-attr` (governs inline `style` attributes).
|===
...
|===
| Method | Returns | Description
| `frameAncestors(...sources)` | `Csp` | Unions sources into `frame-ancestors` — which origins may embed this page in a frame.
| `baseUri(...sources)` | `Csp` | Unions sources into `base-uri` — what the document's `+<base>+` element may set.
| `formAction(...sources)` | `Csp` | Unions sources into `form-action` — where forms may submit.
|===

[.lead]
Baseline and low-level
...
| `add(directive, ...sources)` | `Csp` | Unions sources into any directive by name (deduped). With no sources, registers a boolean directive such as `upgrade-insecure-requests`. A `'nonce-…'` source is rejected.
| `override(directive, ...sources)` | `Csp` | Replaces a directive's source list with exactly these sources, overriding what other contributors set. Not frozen — a later `add()` still extends it.
| `reset(...directives)` | `Csp` | Removes the named directives (for example to unset a boolean directive). With no argument, removes nothing.
| `resetTo(headerValue?)` | `Csp` | Replaces the whole policy with the directives parsed from a raw header string; later contributions still apply on top. Parsing is lenient, mirroring the browser — invalid tokens are skipped, `'nonce-…'` sources are dropped, and of repeated directives only the first counts. A `null`, `undefined`, empty or blank value clears the policy. A value carrying several comma-separated policies is honored (the browser enforces each).
|===
