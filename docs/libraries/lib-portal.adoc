...
Returns the request-scoped <<csp-type, Content Security Policy>> builder for the current portal request. Takes no arguments.

One policy is shared for the whole request: each call returns a new handle, but every handle backs the same policy, so a page together with its layouts, parts and any <<web/sites/processors#, response processors>> can each contribute to the final header. The `Content-Security-Policy` header is composed and emitted automatically when the response is flushed, so contributions made late during rendering still land — you never set the header yourself.

The builder is the way to declare a policy. For backward compatibility the platform still honors a `Content-Security-Policy` header set directly in a controller response — instead of being sent verbatim it is folded into the request policy (parsed as if passed to `resetTo()`), so the policy stays the single source of truth and later contributions still apply on top. A `Content-Security-Policy-Report-Only` header is folded the same way into the request's report-only policy.
...
+*object* : (<<csp-type, `Csp`>>) The Content Security Policy bound to the current portal request.+
...
A request-scoped Content Security Policy builder, obtained from <<csp, `csp()`>>. The same instance backs the whole request, so every controller, layout, part and processor contributes to one emitted `Content-Security-Policy` header. Every method except the nonce getters returns the same `Csp` instance, so calls chain.
