...
Media URLs have one wrinkle worth knowing. A site serves the image and attachment APIs itself, so their URLs sit under the site's endpoint segment — `+<site>/_/media:image/…+`. Where those APIs are not mounted, such as a webapp that has not declared them, URLs fall back to the installation's `+/api/+` space instead: `+/api/media:image/…+`. An operator can also point all media URLs at a separate host — a CDN, typically — with the `+media.defaultBaseUrl+` setting, and can switch off the implicit mounting on sites with `+legacy.mediaApiAutoMount.enabled+`; both live in the portal configuration described in the https://developer.enonic.com/docs/platform/xp8/config[platform configuration reference]. Generated URLs follow whatever is configured, which is the reason to generate them through these functions rather than assembling the paths yourself.
...
| baseUrl | string | *Optional.* Override the resolved base URL. The API is addressed under its endpoint segment: `+<baseUrl>/_/<app>:<api>+`. An empty string counts as not set.
|===
...
| baseUrl | string | *Optional.* Serve the media from this base URL instead of the one resolved from the request: `+<baseUrl>/_/media:attachment/…+`. An empty string counts as not set.
| params | object | *Optional.* Custom query parameters to append to the URL.
|===
...
| project | string | *Optional.* Name of the project.
| branch | string | *Optional.* Name of the branch.
| baseUrl | string | *Optional.* Serve the media from this base URL instead of the one resolved from the request: `+<baseUrl>/_/media:image/…+`. An empty string counts as not set.
| params | object | *Optional.* Custom query parameters to append to the URL.
|===
