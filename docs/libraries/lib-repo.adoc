...
TypeScript types come from the `@enonic-types/lib-repo` package, installed at the version of XP your project builds against — see <<../fundamentals/typescript#, TypeScript>>.
...
|===
| Name | Type | Description
| mode | string | *Optional.* Index type to be refreshed. One of `all`, `search`, or `storage`. Defaults to `all`.
