...
WARNING: `executeFunction()` is deprecated as of image:xp-810.svg[XP 8.1.0,opts=inline] and will be removed together with the Nashorn engine. It is not supported on the GraalJS engine — a function value cannot leave the script context that created it, so calling it there fails immediately. Submit a <<../background/tasks#, named task>> with <<submittask, `submitTask()`>> instead.
