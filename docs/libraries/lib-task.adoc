...
NOTE: If the code is executed in a cluster, an ansynchronous task will only run on one node, but the answer from these functions will be the same, anywhere in the cluster.
...
TypeScript types come from the `@enonic-types/lib-task` package, installed at the version of XP your project builds against — see <<../tooling/typescript#, TypeScript>>.
...
WARNING: `executeFunction()` is deprecated as of image:xp-810.svg[XP 8.1.0,opts=inline] and will be removed together with the Nashorn engine. Submit a <<../background/tasks#, named task>> with <<submittask, `submitTask()`>> instead.

That deprecation stands on its own, on every engine. On GraalJS — a image:preview.svg[Preview,opts=inline] preview engine, *not for production* — `executeFunction()` is unsupported.
...
| user        | <<../libraries/types#user-key, UserKey>> | Key of the user that submitted the task.
