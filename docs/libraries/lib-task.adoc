...
That deprecation stands on its own, on every engine. On GraalJS — a image:preview.svg[Preview,opts=inline] preview engine, *not for production* — `executeFunction()` is not merely deprecated but unsupported: a function value cannot leave the script context that created it, so the call fails immediately.
