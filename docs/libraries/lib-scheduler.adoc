...
TypeScript types come from the `@enonic-types/lib-scheduler` package, installed at the version of XP your project builds against — see <<../tooling/typescript#, TypeScript>>.
...
| lastTaskId | string | Task id of the last job run. image:xp-810.svg[XP 8.1.0,opts=inline] The task started for the next run receives the same id in its context, see <<../background/scheduler#last-task-id, What a scheduled task receives>>.
|===
...
image:xp-810.svg[XP 8.1.0,opts=inline] A one-time job stays after it has run, so its name cannot be reused to run it a second time. Where the name is used for a single occasion — a publish request keyed by a uuid, say — that serves no purpose and the finished jobs only accumulate; `deleteAfterRun` deletes the job instead.
