...
TypeScript types come from the `@enonic-types/lib-context` package, installed at the version of XP your project builds against — see <<../tooling/typescript#, TypeScript>>.
...
| principals | <<../libraries/types#principal-key, PrincipalKey>>[] | *Optional.* Additional principal keys (users, groups, and roles) to attach to the authentication info for the duration of the callback.
...
|===
| Name | Type | Description
| user | <<../libraries/types#user, User>> | *Optional.* The currently authenticated user. Omitted when no user is logged in.
| principals | <<../libraries/types#principal-key, PrincipalKey>>[] | *Optional.* Principal keys (users, groups, and roles) attached to the authentication info.
|===
...
`get()` merges every store it reads from: attributes set on the context — by `run()` or by the platform — win over local-scope attributes with the same key, which in turn win over session attributes. Attributes the platform sets are documented where they apply; a task started by the scheduler, for instance, finds the previous run's task id in `schedule.lastTaskId`, see <<../background/scheduler#last-task-id, What a scheduled task receives>>. And unlike attributes passed to `run()`, which are scoped to the callback, a local-scope attribute stays readable after the call that wrote it returns.
