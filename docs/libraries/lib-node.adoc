...
Use <<libraries/lib-content#, `lib-content`>> for content. Reach for `lib-node` only when working with custom (non-content) repositories. See <<storage/editors#, Editors>> for the pattern.
====
