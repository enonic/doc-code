...
| query | string/object | Query string or <<../storage/querying#, DSL>> expression.
...
| includeChildren | boolean | *Optional.* Indicates that children nodes must be duplicated, too. Default is `true`.
...
`get()` accepts one or multiple strings (node ids or paths), one or multiple arrays of such strings, or one or multiple objects.

When passing objects, each has these properties:
...
| query | string/object | Query string or <<../storage/querying#, DSL>> expression.
| filters | object | *Optional.* Query filters
| sort | string/object | *Optional.* Sorting string or <<../storage/querying#sort, DSL>> expression. Default: '_score DESC'
| aggregations | object | *Optional.* <<../storage/aggregations#,Aggregations>> config
| highlight | object | *Optional.* <<../storage/highlighting#,Highlighting>> config
| explain | boolean | *Optional.* If set to `true`, score calculation explanation will be included in result.
|===
