...
<8> Site service only. HTML fragments contributed to the rendered markup — see <<web/sites/contributions#, page contributions>>.
<9> Site service only. If `false`, no further <<web/sites/processors#, response processors>> or filters run (default `true`).
