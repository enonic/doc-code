...
An HTTP function is a <<../fundamentals/functions#, function>> that responds to an HTTP request. The file where it lives is invoked by one of XP's services (webapp, site, API) based on URL routing defined in descriptors or path conventions.
...
<10> Site service path of the content being rendered. Assigning it in a <<filters#reroute, site mapping filter>> re-routes rendering to another content.
