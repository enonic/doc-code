...
An HTTP function is a <<../runtime/functions#, function>> that responds to an HTTP request. The file where it lives is invoked by one of XP's services (webapp, site, API) based on URL routing defined in descriptors or path conventions.
...
The ``request`` object is passed to every HTTP function. It carries the request method, URL parts, headers, cookies, and body — plus a few XP-specific properties that the site service sets: ``mode`` and ``branch`` for the current rendering mode and repository branch, and ``contentPath`` for the content being rendered.
...
<10> Site service path of the content being rendered. Assigning it in a <<sites/mappings#reroute, site mapping filter>> re-routes rendering to another content.
<11> Request body, as a string, or `null` when absent.
<12> Query and form parameters.
<13> HTTP request headers.
<14> Parsed HTTP request cookies.
<15> Locale strings in decreasing order of preference, derived from the `Accept-Language` header. When the client sends no `Accept-Language` header, the array contains a single element with the server's default locale.
...
<8> Site service only. HTML fragments contributed to the rendered markup — see <<sites/contributions#, page contributions>>.
<9> Site service only. If `false`, no further <<sites/processors#, response processors>> or filters run (default `true`).
