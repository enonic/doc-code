...
* Can mount contextual <<apis#,Universal APIs>> .
* Can connect to <<extensions#,Admin extensions>> by declaring named interfaces that extensions implement.
...
<4> *apis* — list of <<apis#,Universal API>> descriptor keys mounted under this tool's URL. Bare names refer to APIs in the same app; `<app>:<api>` form borrows from another app. APIs become reachable at `+/admin/<app-key>/<tool-name>/_/<app>:<api>+`.
...
The implementation is the entry point for `+/admin/<app-key>/<tool-name>+` and any deeper path that does not match a contextually mounted API. For anything beyond a couple of static endpoints, use the https://market.enonic.com/vendors/enonic/router-lib[router library] for method dispatch, parameter extraction, and pattern matching.

== URL

* Tool entry point: `/admin/<app-key>/<tool-name>`
* Contextual API under the tool: `/admin/<app-key>/<tool-name>/_/<app>:<api-name>`
...
Any API listed in `apis:` is mounted contextually under the tool's URL. The same API can simultaneously be mounted on the Web endpoint (`mount: ["web"]` in the API descriptor) — the two mounts are independent. See <<apis#contextual-apis,Universal API: Contextual APIs>>.
