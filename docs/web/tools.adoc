...
Any API listed in `apis:` is mounted as a service point under the tool's URL. The same API can simultaneously be mounted on the Web endpoint (`mount: ["web"]` in the API descriptor) — the two mounts are independent. See <<web/apis#service-point,Universal API: Service point>>.
