...
The component or HTTP function must add a `pageContributions` property to the <<web/request-response#response, HTTP response object>>.
...
Page contributions are merged into the response markup during a specific step in the <<../sites#, site service>>, called `Contributions filter`. It occurs after <<processors#, page processors>> and before <<mappings#, the site filters>>.
