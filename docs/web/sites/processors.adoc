...
Consider using regular <<web/filters#, HTTP Filters>> for other use-cases.
...
NOTE: If `applyFilters` is set to `false` in the <<web/request-response#response, response object>>, any further response processing or filters will be ignored. Default value is `true`.
