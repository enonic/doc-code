...
The page implementation lives next to its https://developer.enonic.com/docs/cms/xp8/pages/page[Page schema] at `src/main/resources/cms/pages/<page-name>/<page-name>.ts`.
...
A part implementation lives next to its https://developer.enonic.com/docs/cms/xp8/pages/parts[Part schema] at `src/main/resources/cms/parts/<part-name>/<part-name>.ts`.
...
The layout implementation lives next to its https://developer.enonic.com/docs/cms/xp8/pages/parts[Layout schema] at `src/main/resources/cms/layouts/<layout-name>/<layout-name>.ts`.
...
export function GET(req) {
    ...
    return {
        body: render(resolve('./layout-two-column.html'), {
...
[NOTE]
====
By exporting `POST` (or any other HTTP method) from the component implementation, the component can also act as its own backend.

A component can detect whether it is being rendered inline (as part of a page) or directly, via the request context — useful for returning JSON in direct mode while still producing HTML for inline rendering.
====
