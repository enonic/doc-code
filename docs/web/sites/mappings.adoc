...
NOTE: <<web/apis#, Universal APIs>> mounted as service points (`+/_/<app>:<api>/+`) cannot be remapped through site mappings — they are routed before the site engine processes mappings. If you need custom behavior on a service-point URL, implement a custom Universal API instead.
