...
Mappings are the first custom processing step in the <<../sites#, site service>>. Mappings execute before the component rendering. You may define mappings for both <<web/request-response#, HTTP functions>> and <<web/filters#, filters>>. The use cases are vast.
...
<1> *controller* — path to an <<web/request-response#, HTTP function>> implementation.
...
<5> *filter* — path to an <<web/filters#, HTTP filter>> implementation.
...
NOTE: <<web/apis#, Universal APIs>> mounted as points (`+/_/<app>:<api>+`) cannot be remapped through site mappings — they are routed before the site service processes mappings. If you need custom behavior on a service-point URL, implement a custom Universal API instead.
