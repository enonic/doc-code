...
Which mappings apply is decided once, before the first filter runs, from the content originally requested. A filter that <<../filters#reroute, re-routes rendering to another content>> therefore does not change the selection — the mappings already chosen keep running, in order, against the re-routed request.
