...
image:xp-810.svg[XP 8.1.0,opts=inline] An extension that needs the host tool page to allow external resources, or to load a script of its own, registers a Java response processor for its key. It runs after the tool controller and may adjust the page's Content Security Policy or add page contributions. See <<extensions/processors#, Extension response processors>>.
