...
Admin extensions are app-provided HTTP endpoints that mount into existing <<web/tools#,admin tools>> at named interface points. (previously known as *widgets*).
...
See <<web/extensions/liveview#,Liveview>>, <<web/extensions/contextpanel#,Context panel>>, and <<web/extensions/menuitem#,Menu item>> for worked examples: the `contentstudio.liveview`, `contentstudio.contextpanel`, and `contentstudio.menuitem` interfaces published by Content Studio.
