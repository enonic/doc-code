...
An implementation becomes an SSE endpoint by returning an `sse` object from a request handler, such as `GET` or `POST`. Each connected client reuses the same module instance; per-connection state lives in `attributes`.
...
[source,typescript]
----
import {addToGroup, sendToGroup} from '/lib/xp/sse';

export function sseEvent(event) {
    if (event.type === 'open') {
        addToGroup({ group: 'live', clientId: event.clientId });
    }
}
