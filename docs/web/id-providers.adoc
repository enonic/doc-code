...
Rather than hardcoding these paths, generate them with the portal library helpers — <<../libraries/lib-portal#_loginurl, loginUrl()>>, <<../libraries/lib-portal#_logouturl, logoutUrl()>> and <<../libraries/lib-portal#_idproviderurl, idProviderUrl()>> — which resolve the correct contextual URL for the current request.

[NOTE]
====
The endpoint is *mounted* everywhere, but an ID provider only *activates* for a request when it is bound to the request's virtual host. The vhost mapping declares which ID provider(s) apply, and the one marked `default` is the provider whose `autoLogin` filter runs. See the <<../deployment/config#vhost, vhost configuration>> for the binding rules.
====
...
Additionally, an ID provider may also act as a regular <<web/request-response#, HTTP function>> — supporting any interaction as desired.
