...
Access control:: The API must be mounted on the location the request arrived through, and the caller must satisfy its `allow` list — on every request, regardless of endpoint. The <<#descriptor, descriptor>> section lists the resulting status codes.
