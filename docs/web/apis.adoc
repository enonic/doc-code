...
A custom API can act as a WebSocket channel or a Server-Sent Events stream, like any other HTTP function in XP. Return a `webSocket` or `sse` object from the request handler. See <<web/websocket#, WebSocket>> and <<web/sse#, SSE>> for the details.
