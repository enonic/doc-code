...
A Universal API is not tied to a single URL — the same implementation can be reached from the <<#web-endpoint, Web endpoint>> (`+/api/<app>:<api>+`) or as a <<#contextual-apis, contextual API>> inside the site, webapp, and admin services (`+/_/<app>:<api>+`). However it arrives, every request is gated the same way:
...
image::api-pipeline.svg[Universal API request pipeline, 4fix00px]
