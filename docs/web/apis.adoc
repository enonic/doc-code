...
The two endpoints are served on *different ports* and use *different URL forms*: `/api/<app-name>:<api-name>` on the Web endpoint, `/<app-name>:<api-name>` at the root of the Management endpoint. The Management endpoint is not reachable through the Web endpoint and vice versa.
...
APIs are served from the *root* of the Management port, with no `/api` prefix:

* Generic API endpoint: `/<app-name>:<api-name>`

IMPORTANT: The `/api` prefix used on the Web endpoint does not exist on the Management endpoint. Requesting `/api/<app-name>:<api-name>` on the management port returns *404 Not Found* even when the API is mounted and running.
...
`GET /api` (or `GET /api/`) on the Web endpoint returns a JSON index of every API with `"web"` in its `mount` list. The response is a `resources` array, one entry per API:
...
* Discovery is available on the Web endpoint only — the Management endpoint serves no API index.
* Service points (APIs mounted only under sites, webapps, or admin tools) do not appear in the listing.
