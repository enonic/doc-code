= Content Studio Context Panel (contentstudio.contextpanel)
:toc: right
:imagesdir: images
...
A Context Panel extension is declared like any admin extension — descriptor and implementation under `+src/main/resources/admin/extensions/<extension-name>/+`:
...
<3> Optional SVG icon shown next to the panel in the side-panel dropdown.
...
Content Studio renders it inside a web component, which isolates the panel's CSS and JS from the rest of the admin UI:
...
Return `404` if the content can't be loaded; `2xx` otherwise.
