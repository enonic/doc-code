...
The Java object must implement a setter method for each field set that way.

[WARNING]
====
The setter must be called. Assigning to the field as if it were a property is supported by Nashorn only, which rewrote such an assignment into the corresponding setter call; on GraalJS it fails with an `Unknown identifier` error. See <<engines#graaljs-interop, Java interop>>.
