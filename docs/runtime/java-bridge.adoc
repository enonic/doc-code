...
The argument must already be of the type the method declares - it is not coerced on the way in. A JavaScript string passed to a Java `int` or `Long` parameter throws on GraalJS, so parse values that arrive as strings, such as request parameters, before the call. Wrap objects and arrays in `+__.toScriptValue()+` to hand them over as a Java `Map` or `List`.
