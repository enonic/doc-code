...
`app` is the only name *XP* installs on the JavaScript global object. Every other name on this page is *module-scoped*: XP wraps each module in a function and passes them in as arguments. The engine contributes a few globals of its own on top; the <<engines#graaljs, GraalJS>> adds `Graal`.
