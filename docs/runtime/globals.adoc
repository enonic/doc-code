...
`app` is the only name *XP* installs on the JavaScript global object. Every other name on this page is *module-scoped*: XP wraps each module in a function, passing `log`, `require`, `resolve` and `+__+` in as arguments and declaring `exports` and `module` inside it. The engine contributes a few globals of its own on top; <<engines#graaljs, GraalJS>> adds `Graal`.
...
(function (log, require, resolve, __) {
    var exports = {};
    var module = {};
    // module.exports reads and writes the same object as exports
...
* *They can be shadowed.* A variable or parameter named `log` declared in your module hides the binding for the rest of that scope.
...
Assign to it from any module to publish functionality. Assigning to `module.exports` replaces the object as a whole, as in CommonJS.
...
TIP: Authoring in TypeScript, write `export` and let the build transpile it — the emitted JavaScript assigns to `exports`. See <<../tooling/typescript#, TypeScript>>.

[#esm]
== ECMAScript modules are not supported

`import`, `export` and dynamic `import()` do not work at runtime, on either engine. This is a property of XP's module loader, not of the engine: XP evaluates every file as a script wrapped in a function, as shown <<module-scope, above>>, and `import` is only valid in a file evaluated as an ES module. GraalJS can parse the syntax, but XP never loads a file that way.
