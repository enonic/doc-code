...
== Configuration
There are two ways to configure custom scheduled jobs: <<../libraries/lib-scheduler#, lib>> API provides methods to manipulate jobs and https://developer.enonic.com/docs/platform/xp8/deployment/config#scheduler[config] file is used to configure default permanent cron jobs.

== System jobs
XP platform provides predefined system jobs, which can be enabled and overridden by user in https://developer.enonic.com/docs/platform/xp8/deployment/config#scheduler[config] file. Config modification doesn't update job nodes automatically, so make sure you've removed or changed old job nodes manually before any config change.
...
Common vacuum process config can be found in https://developer.enonic.com/docs/platform/xp8/deployment/config#vacuum[Vacuum config file].
...
Common cleanup process config can be found in https://developer.enonic.com/docs/platform/xp8/deployment/config#audit[Audit config file].
