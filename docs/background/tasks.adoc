...
WARNING: Inline function tasks - <<../libraries/lib-task#executefunction, `executeFunction()`>> - are deprecated since XP 8.1 and will be removed together with the Nashorn engine. They are also unsupported on the <<../fundamentals/runtime#graaljs, GraalJS preview>>. Use named tasks; see <<../upgrade/8-1#, Upgrading from XP 8.0 to 8.1>> for how to convert one.
...
NOTE: For fine-grained control which nodes should be capable to execute distributable tasks use https://developer.enonic.com/docs/platform/stable/deployment/config#application[Application] filter config or local apps. Or disable execution of named tasks entirely on some nodes by https://developer.enonic.com/docs/platform/stable/deployment/config#task[Task] config.
