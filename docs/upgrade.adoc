...
`delete` → `deleteContent` / `deleteProject` / …:: Library methods named `delete` were renamed to avoid the reserved word (the old `delete` exports remain for backward compatibility).
