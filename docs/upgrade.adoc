...
`submit` method is removed. Its drop-in replacement `executeFunction` is deprecated in turn (not supported on the GraalJS engine) — convert the callback to a <<background/tasks#, named task>> and use `submitTask` instead.
