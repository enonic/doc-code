...
`submit` method is removed. Its drop-in replacement `executeFunction` is deprecated in turn (not supported on the image:preview.svg[Preview,opts=inline] GraalJS engine, a preview - *not for production*) — convert the callback to a <<background/tasks#, named task>> and use `submitTask` instead.
