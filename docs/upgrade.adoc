...
NOTE: Migrating to Universal API means the existing service endpoint URLs will change. Services were previously mounted under `+/_/service/<app-name>/<service-name>+`, while Universal APIs are available directly under `+/_/<app-name>/<api-name>+`. Additionally, services were automatically mounted on admin tools, web-apps, and sites, while Universal APIs must be explicitly mounted via the respective descriptors.
